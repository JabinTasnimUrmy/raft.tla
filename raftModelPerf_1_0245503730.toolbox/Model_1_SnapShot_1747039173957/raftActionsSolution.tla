...
ClientRequest(switchIdx, i, v) == 
    ...
    /\ UNCHANGED <<messages, serverVars, candidateVars, leaderVars, logVars, leaderCount, entryCommitStats, unorderedClientRequestss, switchIndex, switchToServerSentRecord, Servers>>
...
SwitchReplicateClientRequest(switchIdx, i, v) == 
    ...
    /\ LET entryToReplicate == switchBuffer[v]
       ...
       IN
          /\ valueTermPair \notin switchToServerSentRecord[i]
          /\ unorderedClientRequestss' = [unorderedClientRequestss EXCEPT ![i] = @ \cup {v}] \* we only store value, we can use that as an index to the switch buffer to get the full entry
          /\ switchToServerSentRecord' = [switchToServerSentRecord EXCEPT ![i] = @ \cup {valueTermPair}]
...
LeaderIngestHovercRaftRequest(i, v) ==
    /\ v \in unorderedClientRequestss[i]
    /\ LET
       ...
       IN
        ...
        /\ unorderedClientRequestss' = [unorderedClientRequestss EXCEPT ![i] = @ \ {v}]

    /\ UNCHANGED <<messages, serverVars, candidateVars, leaderVars, commitIndex, leaderCount, maxc, switchBuffer, switchIndex, switchToServerSentRecord, Servers>>
...
AppendEntries(i, j) ==
    ...
    /\ LET entryIndex == nextIndex[i][j]
       ...
       /\ entryCommitStats' =
            ...
            ELSE entryCommitStats         
    /\ UNCHANGED <<serverVars, candidateVars, leaderVars, logVars, maxc, leaderCount, NetAgghovercraftVars, Servers>>
    ...
    LET logOk == \/ m.mprevLogIndex = 0
        ...
        cacheMissHovercRaft ==
          ...
          /\ LET entry == m.mentries[1]
                 ...
                 msgTerm == entry.term
             IN 
             \lnot ( /\ v \in unorderedClientRequestss[i]
    ...
    IN /\ m.mterm <= currentTerm[i]
       /\ \/ /\ \* reject request
             ...
             /\ Reply([mtype           |-> AppendEntriesResponse,
                       ...
                       mdest           |-> j],
                       m)
             /\ UNCHANGED <<serverVars, logVars, unorderedClientRequestss>>
          \/ \* return to follower state
             /\ m.mterm = currentTerm[i]
             /\ state[i] = Candidate
             /\ state' = [state EXCEPT ![i] = Follower]
             /\ UNCHANGED <<currentTerm, votedFor, logVars, messages, unorderedClientRequestss>>
          \/ \* accept request
             /\ m.mterm = currentTerm[i]
             /\ state[i] = Follower
             /\ logOk
...
\*                                                commitIndex[i]]
                       /\ Reply([mtype           |-> AppendEntriesResponse,
                                 ...
                                 mdest           |-> j],
                                 m)
                       /\ UNCHANGED <<serverVars, log, unorderedClientRequestss>>
...
\*                          IN log' = [log EXCEPT ![i] = new]
                       /\ UNCHANGED <<serverVars, commitIndex, messages, unorderedClientRequestss>>
                   \/ \* no conflict: append entry
                       ...
                       /\ LET
                          ...
                          IN 
                             ...
                             /\ unorderedClientRequestss' = [unorderedClientRequestss EXCEPT ![i] = @ \ {entryMetaData.value}] \* remove since it is now ordered


                       /\ UNCHANGED <<serverVars, commitIndex, messages>>
       /\ UNCHANGED <<candidateVars, leaderVars, instrumentationVars, switchIndex, switchBuffer, switchToServerSentRecord, Servers>> \* entryCommitStats unchanged on followers
